...
- 第一版同时完成游戏模式和桌面游戏矩阵；达到双模式验收后停止扩展功能。
...
当前结果：离线实现和自动化验收已完成。内容寻址运行时、逐文件清单、同版本重打包、事务回滚、更新中断恢复、待确认卸载、最终清理、系统 MangoApp 回退、控制器和 Decky 页面均有回归测试。真实 Loader 更新与卸载路径留到阶段 5。

## 阶段 5：双模式真实设备验收

1. 在桌面模式用双模式正式 zip 和真实 systemd 用户管理器验收安装事务、socket activation 与代理。
2. 用正式 `launcher.py desktop -- %command%` 从 Steam 启动 KDE 原生/Proton 游戏，验收 OpenGL、Vulkan、图片、GIF、resize 和全屏。
3. 桌面正式包通过后，用同一个 Decky 包切换一次游戏模式。
4. 通过 SSH 热重启 MangoApp 服务，完成 Steam 首页、游戏、QAM 和性能层级矩阵。
5. 验证两种模式中渲染器退出、代理崩溃和自动恢复都不改变安装状态。
6. 验证不同分辨率、安全区、挂起恢复和开机启动；外接屏在无硬件时明确延期。
7. 与基线比较空载和典型场景开销，确定发布配额。

完成条件：调试文档中的最低验收矩阵全部通过，并形成用户可执行的测试清单。

当前状态：进行中。旧正式包已在真实游戏模式完成激活、更新和最终卸载验证；控制器、`XDG_RUNTIME_DIR`、同版本重打包、字体和完整输出窗口问题均已修复。KDE 桌面双 ABI、SteamRT4、Proton、resize、全屏、多进程、多 swapchain 与真实挂起恢复矩阵已通过。当前正在把这些桌面产物纳入同一个 Decky 正式包；下一人工步骤是先测试该包的桌面启动器，通过后再用同一包回归游戏模式。真实外接输出因当前没有硬件延期，固定功耗真实游戏的发布预算仍待完成。
...
当前准备：确定性 Decky zip、内容寻址运行时、Steam Runtime 4 双模式构建、包内 C/C++/Python/Rust SDK 验收和项目专用 CI 已纳入发布链。真实桌面到游戏模式顺序验收完成前不创建首版 release。
...
KDE 桌面常驻覆盖窗口、输入交互、视频、SVG、自定义字体和自定义着色器不属于第一版。
