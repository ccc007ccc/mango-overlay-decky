...
## 阶段 7：多产品共享 Runtime Coordinator

该阶段作为共享基础库在第三方 Decky 产品接入前完成：
...
当前结果：1–5 已完成并有 19 项协调器测试、生命周期集成测试和 launcher 命令测试；
一百个顺序声明只比较有界元数据，稳定选择不调用 verifier，新增最大版本只验证一次，
更新/失败回退/非最后声明卸载/最后声明卸载均通过公共 interface 测试。第三方产品的
真实 Decky 更新路径仍需在阶段 5 的设备验收中复测。同一产品后端重载在不重启渲染器的
情况下刷新 generation；非活动产品也能留下自己的待确认卸载记录，避免共享核心的全局
安装状态阻塞其他声明。
