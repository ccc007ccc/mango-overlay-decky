...
当前状态：进行中。旧正式包已在真实游戏模式完成激活、更新和最终卸载验证；随后实机日志发现旧卸载回调会超过 Loader 的五秒停止期限，修复和 53 项完整插件测试已经完成，尚待用新包复测“数秒卸载”和正常更新不误清理。控制器、`XDG_RUNTIME_DIR`、同版本重打包、字体和完整输出窗口问题均已修复。桌面注入因要求逐游戏启动项且会覆盖 Steam 原生统计，已按 ADR 0016 移出首发。下一人工步骤是安装新的 game-mode-only 包并直接在真实游戏模式回归。真实外接输出因当前没有硬件延期，固定功耗真实游戏的发布预算仍待完成。
